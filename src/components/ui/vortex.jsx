...
export const Vortex = (props) => {
    ...
    const initParticle = (i) => {
        ...
        let y = formTopY() + randRange(rangeY);
        let vx = 0;
        let vy = 0;
        let life = 0;
        ...
        let hue = baseHue + rand(rangeHue) - rangeHue / 2;
    ...
    const drawParticle = (x, y, x2, y2, life, ttl, radius, hue, ctx) => {
        const fadeByY = Math.min(1, Math.max(0, (y - formTopY()) / rangeY));
        ctx.save();
        ctx.lineCap = "round";
        ctx.lineWidth = radius;
        ctx.strokeStyle = `hsla(${hue},100%,50%,${fadeInOut(life, ttl) * fadeByY})`;
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(x2, y2);
        ctx.stroke();
